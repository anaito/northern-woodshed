...
function houseIcon(fillHex, strokeHex, strokeWidth, sizePx) {
    ...
    var svg =
        ...
        // Drop shadow: dx/dy ~ 3–4px, blur ~ 2.5
        '<defs>'
        + '<filter id="nwDropShadow" x="-50%" y="-50%" width="200%" height="200%">'
        + '<feDropShadow dx="3" dy="3" stdDeviation="2.2" flood-color="#000000" flood-opacity="0.35"/>'
        + '</filter>'
        + '</defs>'
        +

        // Apply filter to a group containing roof+body
        '<g filter="url(#nwDropShadow)">'
        + roof + body
        + '</g>'
        +
    ...
    return fetch(url)
        ...
        .then(function (data) {
            var layer = L.geoJSON(data, {
                ...
                pointToLayer: function (feature, latlng) {
                    return L.marker(latlng, {
                        icon: houseIcon(fillHex || "#FE9829", "#000000", 1, 20)
                    });
                },
                onEachFeature: function (feature, layer) {
            ...
            var opsSw = houseIcon(fillHex || "#FE9829", "#000000", 1, 14).options.html;
            groupedControl.addOverlay(
                layer,
