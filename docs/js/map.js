...
function loadForestryOperatorsFile(url, label) {
    return fetch(url)
        ...
        .then(function (data) {
            ...
            groupedControl.addOverlay(layer, label, "Facilities");
            refreshLayerControl();
        })
        .catch(function (err) {
            alert(err.message);
    ...
    return fetch("data/woodchipSuppliers.geojson")
        ...
        .then(function (data) {
            Object.keys(woodchipSymbology).forEach(function (k) {
                ...
                groupedControl.addOverlay(subLayer, entry.label, "Facilities");
            });
...
function loadRipley() {
    return fetch("data/ripley.geojson")
        ...
        .then(function (data) {
            ...
            groupedControl.addOverlay(ripleyLayer, "Harvey G. Ripley Heating Plant");
            refreshLayerControl();

            // Optional: show by default
...
legend.onAdd = function () {
    ...
    var ripleySw = ripleyIcon("#FFFFFF", "#000000", "#000000", 35).options.html;
    div.innerHTML +=
    ...
    // Facilities (house icon)
    div.innerHTML += '<hr style="margin:8px 0;">';
    div.innerHTML += '<div class="legend-title"Facilities</div>';
