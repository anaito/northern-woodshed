...
// --------------------
// Layer control
// Mobile: collapsed by default (so it doesn't cover the screen)
// Desktop: always open (no collapse UI when collapsed=false)
// --------------------
...
// Helper: reliably collapse the layers control (plugin-safe)
function nwCollapseLayersControl(ctrl) {
    if (ctrl && typeof ctrl.collapse === 'function') {
        ctrl.collapse();
        return;
    }
    if (ctrl && typeof ctrl._collapse === 'function') {
        ctrl._collapse();
        return;
    }

    var container = ctrl && (ctrl.getContainer ? ctrl.getContainer() : ctrl._container);
    if (!container) return;

    L.DomUtil.removeClass(container, 'leaflet-control-layers-expanded');
}

// Mobile-only: add a "Close layers" button inside the expanded layers panel
(function addMobileCloseButton() {
    var isMobileHere = window.matchMedia && window.matchMedia('(max-width: 768px)').matches;
    if (!isMobileHere) return;
    ...
    function ensureButton() {
        ...
        if (container.querySelector('.nw-mobile-close-layers')) return;

        var form = container.querySelector('form');
        var btn = L.DomUtil.create('button', 'nw-mobile-close-layers');
        btn.type = 'button';
        btn.textContent = 'Close layers';
        ...
        btn.addEventListener('click', function (e) {
            L.DomEvent.stop(e);
            nwCollapseLayersControl(groupedControl);
        });
...
// Allow scrolling inside the layers control on mobile (prevent map from panning)
(function enableLayersControlScroll() {
    var el = groupedControl.getContainer();
    if (!el) return;
    ...
    L.DomEvent.on(el, 'touchmove', function (e) {
        L.DomEvent.stopPropagation(e);
    });
})();
...
// --------------------
// Woodshed draw-order helpers
// --------------------
var woodshedLayersCache = null;
var _nwWoodshedReorderScheduled = false;

function bringGeoJsonGroupToBack(gj) {
    if (!gj) return;

    if (typeof gj.bringToBack === 'function') {
        gj.bringToBack();
        return;
    }

    if (typeof gj.eachLayer === 'function') {
        gj.eachLayer(function (l) {
            if (l && typeof l.bringToBack === 'function') {
                l.bringToBack();
            }
        });
    }
...
function reorderWoodshedZ() {
    if (!woodshedLayersCache) return;

    // woodshedLayersCache order is: [25, 50, 75, 100]
    // Bring-to-back in this order yields final stack:
    // bottom -> top = 100, 75, 50, 25
    [0, 1, 2, 3].forEach(function (idx) {
        var lyr = woodshedLayersCache[idx];
        if (!lyr || !map.hasLayer(lyr)) return;
        bringGeoJsonGroupToBack(lyr);
    });
}

function scheduleWoodshedReorder() {
    if (_nwWoodshedReorderScheduled) return;
    _nwWoodshedReorderScheduled = true;

    var raf = window.requestAnimationFrame || function (cb) { return setTimeout(cb, 0); };

    raf(function () {
        raf(function () {
            _nwWoodshedReorderScheduled = false;
            reorderWoodshedZ();
        });
    });
}
...
var wildlifeSymbology = {
    "american marten": { label: "American marten", shape: "diamond", rx: 0.55, fill: "#3265AF", stroke: "#FFFFFF", strokeWidth: 1 },
    ...
    "snowshoe hare": { label: "Snowshoe hare", shape: "hexagon", rotateDeg: 30, fill: "#712F84", stroke: "#FFFFFF", strokeWidth: 1 }
};
...
function svgIconFor(entry, sizePx) {
    ...
    if (entry.shape === "circle") {
        ...
        var r2 = s / 2;
        var deg = (entry.rotateDeg !== undefined) ? entry.rotateDeg : 30;
        var rot = deg * Math.PI / 180;

        var pts = [];
        for (var i = 0; i < 6; i++) {
            var a = (i * 60) * Math.PI / 180 + rot;
            var xh = cx + r2 * Math.cos(a);
            var yh = cy + r2 * Math.sin(a);
            pts.push(xh + "," + yh);
        }

        shapeSvg = '<polygon points="' + pts.join(" ")
            + '" fill="' + fill + '" stroke="' + stroke + '" stroke-width="' + sw + '"/>';
    } else {
        var rFallback = (s / 2) - 1;
...
// --------------------------------------------------
// Facilities operators — house icon
// --------------------------------------------------
...
// z-index offsets for consistent overlap within Facilities
var NW_Z_FACILITY_OPERATORS = 0;     // 2014/2025
var NW_Z_WOODCHIP_SUPPLIERS = 1000;  // always on top of operators

// UPDATED: added fillHex parameter for 2014 vs 2025
function loadForestryOperatorsFile(url, label, fillHex) {
    var fill = fillHex || "#FE9829";
    ...
    return fetch(url)
        ...
        .then(function (data) {
            var layer = L.geoJSON(data, {
                ...
                pointToLayer: function (feature, latlng) {
                    return L.marker(latlng, {
                        icon: houseIcon(fill, "#000000", 1, 20),
                        zIndexOffset: NW_Z_FACILITY_OPERATORS
                    });
                },
                onEachFeature: function (feature, layer) {
            ...
            var opsSw = houseIcon(fill, "#000000", 1, 14).options.html;
            groupedControl.addOverlay(
                layer,
                ...
                var subLayer = L.geoJSON(data, {
                    ...
                    pointToLayer: function (feature, latlng) {
                        return L.marker(latlng, {
                            ...
                            zIndexOffset: NW_Z_WOODCHIP_SUPPLIERS
                        });
                    },
                    onEachFeature: function (feature, layer) {
...
// 1) Ripley (Reference)
// 2) Facilities  (WOODCHIP FIRST so it appears at top in the control)
// 3) Wildlife
...
loadRipley()
    .then(function () {
        // Facilities: add woodchip suppliers FIRST (top of Facilities list)
        return loadWoodchipSuppliers()
            .then(function () {
                // Then add Forestry operators (below in Facilities list)
                return loadForestryOperatorsFile(
                    "data/forestryOperators2014.geojson",
                    "Forestry operators present in 2014",
                    "#969696"
                );
            })
            .then(function () {
                return loadForestryOperatorsFile(
                    "data/forestryOperators2025.geojson",
                    "Forestry operators present in 2025",
                    "#636363"
                );
            });
    })
    ...
    .then(function (layers) {
        ...
        layers.forEach(function (lyr) {
            lyr.on('add', scheduleWoodshedReorder);
            lyr.on('remove', scheduleWoodshedReorder);
        });
        ...
        groupedControl.addOverlay(layers[0], labelWithColorSwatch("#B3CDE3", "25 mi distance from Ripley Heating Plant", 0.20), 'Woodshed Extents');
        groupedControl.addOverlay(layers[1], labelWithColorSwatch("#8C96C6", "50 mi distance from Ripley Heating Plant", 0.20), 'Woodshed Extents');
        groupedControl.addOverlay(layers[2], labelWithColorSwatch("#8856A7", "75 mi distance from Ripley Heating Plant", 0.20), 'Woodshed Extents');
        groupedControl.addOverlay(layers[3], labelWithColorSwatch("#810F7C", "100 mi distance from Ripley Heating Plant", 0.20), 'Woodshed Extents');

        map.fitBounds(layers[3].getBounds(), { padding: [50, 50] });
        refreshLayerControl();

        scheduleWoodshedReorder();
