...
// --------------------
// Panes (guaranteed draw order)
// Woodshed extents always at the bottom, regardless of toggle order.
// Default overlay pane sits higher (~400), so 200 stays safely below.
// --------------------
...
function nwCollapseLayersControl(ctrl) {
    // Try public-ish methods first (some builds expose these)
    ...
    // Fallback: remove the expanded class
...
(function addMobileCloseButton() {
    ...
    function ensureButton() {
        ...
        // Only when expanded (so it doesn't appear when collapsed)
        ...
        // Insert after the form (nice layout)
        ...
        btn.addEventListener('click', function (e) {
            L.DomEvent.stop(e); // prevent any map interaction
    ...
    // Run now, and also whenever the control gets expanded later
    ensureButton();

    // Observe class changes (expanded/collapsed) and re-inject button if DOM rebuilds
    var container = groupedControl.getContainer();
...
(function enableLayersControlScroll() {
    ...
    // Stop clicks/scrolls from affecting the map
    L.DomEvent.disableClickPropagation(el);
    L.DomEvent.disableScrollPropagation(el);

    // Critical on touch devices: prevent touch-drag from panning the map
...
// --------------------
// Added optional paneName so woodshed extents can be forced into a bottom pane.
    ...
    return fetch(url)
        ...
        .then(function (data) {
            return L.geoJSON(data, {
                pane: paneName,   // <-- NEW (safe if undefined)
                style: styleFn,
...
loadRipley()
    ...
    .then(function () {
        // Woodshed polygons in their dedicated bottom pane
        return Promise.all([
