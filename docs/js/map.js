...
// --------------------------------------------------
function normalizeOwnership(value) { return normalizeText(value); }
...
// --------------------------------------------------
function normalizeForestType(value) { return normalizeText(value); }
...
var wildlifeSymbology = {
  ...
  "snowshoe hare":   { label: "Snowshoe hare",   shape: "hexagon", fill: "#FFFA56", stroke: "#FFFFFF", strokeWidth: 1 }
};
...
// --------------------------------------------------
// Forestry operators — points, house icon
// File: data/forestryOperators.geojson
// Attribute: millType OR forestryOperatorsType (both supported)
// --------------------------------------------------
...
function loadForestryOperators() {
  return fetch("data/forestryOperators.geojson")
    .then(function (r) {
      if (!r.ok) throw new Error("Forestry operators fetch failed: " + r.status + " " + r.statusText);
      return r.json();
    })
    .then(function (data) {
      var opsLayer = L.geoJSON(data, {
        filter: function (feature) {
          // Support either field name, so you don't have to re-export just to rename
          var props = feature.properties || {};
          var t = props.forestryOperatorsType !== undefined ? props.forestryOperatorsType : props.millType;
        ...
        onEachFeature: function (feature, layer) {
          layer.bindPopup("<strong>Forestry operators</strong><br><strong>Type:</strong> business");
        }
      });

      groupedControl.addOverlay(opsLayer, "Forestry operators", "Infrastructure");
      refreshLayerControl();
    })
    .catch(function (err) { alert(err.message); });
}
...
legend.onAdd = function () {
  ...
  Object.keys(ownershipSymbology).forEach(function (k) {
    ...
    div.innerHTML +=
      '<div class="legend-item"><span class="swatch" style="background:' + e.color + '"></span>' +
      e.label + '</div>';
  });
  ...
  Object.keys(forestTypeSymbology).forEach(function (k2) {
    ...
    div.innerHTML +=
      '<div class="legend-item"><span class="swatch" style="background:' + e2.color + '"></span>' +
      e2.label + '</div>';
  });
