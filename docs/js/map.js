...
// Mobile-only: add a "Close" button inside the expanded layers panel
(function addMobileCloseButton() {
    ...
    function ensureButton() {
        ...
        btn.addEventListener('click', function (e) {
            e.preventDefault();
            if (typeof groupedControl.collapse === 'function') {
                groupedControl.collapse();
            }
        });
    ...
    // When user taps the toggle icon, Leaflet adds "expanded" class—observe and inject button
    var container = groupedControl.getContainer();
...
function svgIconFor(entry, sizePx) {
    ...
    if (entry.shape === "circle") {
        ...
        var ry = (s / 2); // vertical radius
        var rx = (s / 2) * (entry.rx || 0.60); // horizontal radius (narrower)
        ...
        for (var i = 0; i < 6; i++) {
            ...
            var x = cx + r2 * Math.cos(a);
            var y = cy + r2 * Math.sin(a);
            pts.push(x + "," + y);
        }
