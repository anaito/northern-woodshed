    ...
    return fetch("data/commercial_forest.geojson")
        ...
        .then(function (data) {
            Object.keys(ownershipSymbology).forEach(function (key) {
                ...
                groupedControl.addOverlay(subLayer, entry.label, "Commercial Forest Ownership Type");
            });
    ...
    return fetch("data/dnr_forest_type.geojson")
        ...
        .then(function (data) {
            ...
            Object.keys(forestTypeSymbology).forEach(function (key) {
                ...
                groupedControl.addOverlay(subLayer, entry.label, "MiDNR Vegetation Type");
            });
...
function loadForestryOperatorsFile(url, label) {
    return fetch(url)
        ...
        .then(function (data) {
            ...
            groupedControl.addOverlay(layer, label, "Infrastructure");
            refreshLayerControl();
        })
        .catch(function (err) {
            alert(err.message);
...
// --------------------------------------------------
// Woodchip suppliers — house icon (identical symbol to forestry operators)
// File: data/woodchipSuppliers.geojson
// Field: woodType (H/S)
// --------------------------------------------------
    ...
    return fetch("data/woodchipSuppliers.geojson")
        ...
        .then(function (data) {
            Object.keys(woodchipSymbology).forEach(function (k) {
                ...
                groupedControl.addOverlay(subLayer, entry.label, "Infrastructure");
            });
...
function ripleyIcon(fillHex, strokeHex, donutStrokeHex, sizePx) {
    var size = sizePx || 27; // ~1.5x other point symbols (18px)
    var box = size;
    var cx = box / 2;
    ...
    // Simple pin path + donut ring (donut is transparent with stroke)
    var svg =
    ...
    return L.divIcon({
        ...
        html: svg,
        iconSize: [box, box],
        iconAnchor: [box / 2, box] // bottom tip anchored to location
    });
}
...
function loadRipley() {
    return fetch("data/ripley.geojson")
        ...
        .then(function (data) {
            ...
            // Add above woodshed in the control by registering it before woodshed overlays
            groupedControl.addOverlay(ripleyLayer, "Harvey G. Ripley Heating Plant", "Reference");
            refreshLayerControl();

            // Optional: show by default (comment out if you prefer it off)
...
loadRipley().then(function () {
    ...
    groupedControl.addOverlay(layers[0], '25 mi distance from Ripley Heating Plant', 'Woodshed Extents');
    groupedControl.addOverlay(layers[1], '50 mi distance from Ripley Heating Plant', 'Woodshed Extents');
    groupedControl.addOverlay(layers[2], '75 mi distance from Ripley Heating Plant', 'Woodshed Extents');
    groupedControl.addOverlay(layers[3], '100 mi distance from Ripley Heating Plant', 'Woodshed Extents');
...
legend.onAdd = function () {
    ...
    // Ripley (above Woodshed)
    var ripleySw = ripleyIcon("#FFFFFF", "#000000", "#000000", 18).options.html;
    div.innerHTML += '<div class="legend-title">Harvey G. Ripley Heating Plant</div>';
    div.innerHTML +=
        '<div class="legend-item"><span style="display:inline-block; width:22px; height:22px; margin-right:8px;">'
        + ripleySw + '</span>Harvey G. Ripley Heating Plant</div>';

    // Woodshed
    div.innerHTML += '<hr style="margin:8px 0;">';
    div.innerHTML += '<div class="legend-title">Woodshed Extent</div>';
    [{
            label: '25 mi from Ripley Heating Plant',
            color: '#B3CDE3'
        },
        {
            label: '50 mi from Ripley Heating Plant',
            color: '#8C96C6'
        },
        {
            label: '75 mi from Ripley Heating Plant',
            color: '#8856A7'
        },
        {
            label: '100 mi from Ripley Heating Plant',
            color: '#810F7C'
        }
    ].forEach(function (d) {
        div.innerHTML += '<div class="legend-item"><span class="swatch" style="background:' + d.color + '"></span>' + d.label + '</div>';
    });

    // Commercial Forest
    div.innerHTML += '<hr style="margin:8px 0;">';
    div.innerHTML += '<div class="legend-title">Commercial Forest Ownership Type</div>';
    Object.keys(ownershipSymbology).forEach(function (k) {
        var e = ownershipSymbology[k];
        div.innerHTML += '<div class="legend-item"><span class="swatch" style="background:' + e.color + '"></span>' + e.label + '</div>';
    });

    // DNR Veg Type
    div.innerHTML += '<hr style="margin:8px 0;">';
    div.innerHTML += '<div class="legend-title">MiDNR Vegetation Type</div>';
    Object.keys(forestTypeSymbology).forEach(function (k2) {
        var e2 = forestTypeSymbology[k2];
        div.innerHTML += '<div class="legend-item"><span class="swatch" style="background:' + e2.color + '"></span>' + e2.label + '</div>';
    });
    ...
    Object.keys(wildlifeSymbology).forEach(function (k3) {
        ...
        div.innerHTML +=
            '<div class="legend-item"><span style="display:inline-block; width:18px; height:18px; margin-right:8px;">'
            + sw + '</span>' + e3.label + '</div>';
    });

    // Infrastructure (house icon)
    div.innerHTML += '<hr style="margin:8px 0;">';
    ...
    div.innerHTML +=
        '<div class="legend-item"><span style="display:inline-block; width:18px; height:18px; margin-right:8px;">'
        + opsSw + '</span>Forestry operators present in 2014</div>';

    div.innerHTML +=
        '<div class="legend-item"><span style="display:inline-block; width:18px; height:18px; margin-right:8px;">'
        + opsSw + '</span>Forestry operators present in 2025</div>';

    // Woodchip suppliers
    var hcSw = houseIcon("#38A700", "#000000", 1, 14).options.html; // hardwood = green
    var scSw = houseIcon("#A87001", "#000000", 1, 14).options.html; // softwood = brown

    div.innerHTML +=
        '<div class="legend-item"><span style="display:inline-block; width:18px; height:18px; margin-right:8px;">'
        + hcSw + '</span>Hardwood woodchip supplier</div>';

    div.innerHTML +=
        '<div class="legend-item"><span style="display:inline-block; width:18px; height:18px; margin-right:8px;">'
        + scSw + '</span>Softwood woodchip supplier</div>';
