...
// --------------------------------------------------
// Woodchip suppliers — circle markers
    ...
    .then(function (data) {
      Object.keys(woodchipSymbology).forEach(function (k) {
        ...
        var subLayer = L.geoJSON(data, {
          filter: function (feature) {
            var props = feature.properties || {};
            return normalizeText(props.woodType) === k; // "H"/"S" -> "h"/"s"
          },
          pointToLayer: function (feature, latlng) {
...
legend.onAdd = function () {
  ...
  // Infrastructure (house icon + woodchip suppliers)
  div.innerHTML += '<hr style="margin:8px 0;">';
  ...
  div.innerHTML +=
    '<div class="legend-item"><span class="swatch" style="background:#A87001"></span>Hardwood woodchip supplier</div>';
  div.innerHTML +=
    '<div class="legend-item"><span class="swatch" style="background:#38A700"></span>Softwood woodchip supplier</div>';
