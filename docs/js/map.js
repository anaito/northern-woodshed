...
// --------------------
// Panes (force draw order)
// woodshed always at bottom; polygons above; points on top
// --------------------
map.createPane('woodshedPane');
map.getPane('woodshedPane').style.zIndex = 200;
...
var groupedControl = L.control.groupedLayers(baseMaps, {}, {
    // Desktop: enable collapsible mode but we will NOT use hover; we will use our footer button.
    // Mobile: keep expanded when shown (your CSS toggles visibility)
    collapsed: isMobile ? false : true,
    position: 'topleft'
}).addTo(map);

// Mobile-only Layers toggle button
var layersToggle = L.control({ position: 'topleft' });
layersToggle.onAdd = function () {
    ...
    btn.addEventListener('click', function () {
        document.body.classList.toggle('nw-layers-open');
    });
...
// IMPORTANT: allow scrolling inside the control
var ctlEl = groupedControl.getContainer();
L.DomEvent.disableClickPropagation(ctlEl);
L.DomEvent.disableScrollPropagation(ctlEl);

// --------------------
// Desktop: disable hover open/close + hide default toggle + add footer button
// --------------------
function nwSetupDesktopLayerControlToggle() {
    if (isMobile) return;

    var container = groupedControl.getContainer();
    if (!container) return;

    // 1) Disable Leaflet hover behavior (mouseenter/mouseleave auto expand/collapse)
    //    Leaflet adds these when collapsed=true on non-touch devices.
    L.DomEvent.off(container, 'mouseenter', groupedControl.expand, groupedControl);
    L.DomEvent.off(container, 'mouseleave', groupedControl.collapse, groupedControl);
    L.DomEvent.off(container, 'focus', groupedControl.expand, groupedControl);
    L.DomEvent.off(container, 'blur', groupedControl.collapse, groupedControl);

    // 2) Hide the default Leaflet toggle button/area (the "extraneous" thing)
    var defaultToggle = container.querySelector('.leaflet-control-layers-toggle');
    if (defaultToggle) {
        defaultToggle.style.display = 'none';
        defaultToggle.setAttribute('aria-hidden', 'true');
        defaultToggle.tabIndex = -1;
    }

    // 3) Ensure our footer button exists (and is not duplicated)
    if (!container.querySelector('.nw-layerctl-footer-toggle')) {
        // Put the button AFTER the form if possible (more stable with grouped plugin updates)
        var form = container.querySelector('form');
        var btn = L.DomUtil.create('button', 'nw-layerctl-footer-toggle');
        btn.type = 'button';
        ...
        btn.addEventListener('click', function (e) {
            ...
            if (container.classList.contains('leaflet-control-layers-expanded')) {
                if (typeof groupedControl.collapse === "function") groupedControl.collapse();
            } else {
                if (typeof groupedControl.expand === "function") groupedControl.expand();
            }
            nwSyncDesktopLayerToggleText();
        });
    }

    // Start OPEN (but still collapsible)
    if (typeof groupedControl.expand === "function") groupedControl.expand();
    nwSyncDesktopLayerToggleText();
}

function nwSyncDesktopLayerToggleText() {
    if (isMobile) return;

    var container = groupedControl.getContainer();
    if (!container) return;

    var btn = container.querySelector('.nw-layerctl-footer-toggle');
    if (!btn) return;

    btn.textContent = container.classList.contains('leaflet-control-layers-expanded')
        ? 'Close Layer Control'
        : 'Open Layer Control';
}

// Run once after the control is created
nwSetupDesktopLayerControlToggle();
...
function refreshLayerControl() {
    ...
    // Re-apply desktop toggle + resync label in case the control DOM was rebuilt
    if (!isMobile) {
        nwSetupDesktopLayerControlToggle();
        nwSyncDesktopLayerToggleText();
    }
...
function applyGroupCollapsers() {
    var container = groupedControl.getContainer();
    if (!container) return;

    var groupSpans = container.querySelectorAll('.leaflet-control-layers-group-name');
    groupSpans.forEach(function (span) {
        var groupName = (span.textContent || "").trim();

        // Exclusions: do NOT collapse these groups
        if (groupName === "Reference" || groupName === "Woodshed Extents") return;

        var groupDiv = span.closest('.leaflet-control-layers-group');
        if (!groupDiv || groupDiv.dataset.nwCollapsibleApplied === "1") return;

        groupDiv.dataset.nwCollapsibleApplied = "1";

        // Default collapsed state for selected groups (you can adjust this list)
        var defaultCollapsed =
            groupName === "Facilities"
            || groupName === "Wildlife Observations"
            || groupName === "Commercial Forest Ownership Type"
            || groupName === "MiDNR Vegetation Type";

        if (defaultCollapsed) groupDiv.classList.add('nw-collapsed');

        // Toggle on click of the group label row
        var labelEl = groupDiv.querySelector('.leaflet-control-layers-group-label');
        var bodyEl = groupDiv.querySelector('.leaflet-control-layers-group-items');
        if (!labelEl || !bodyEl) return;

        labelEl.addEventListener('click', function (evt) {
            // Don’t toggle when clicking a checkbox inside the group
            if (evt.target && (evt.target.tagName === 'INPUT' || evt.target.tagName === 'LABEL')) return;
            groupDiv.classList.toggle('nw-collapsed');
        });
    });
}
...
// --------------------
function loadGeoJson(url, styleFn, label, paneName) {
    return fetch(url)
        .then(function (r) {
...
function loadDnrForestType() {
    return fetch("data/dnr_forest_type.geojson")
        ...
        .then(function (data) {
            ...
            var buckets = {};
            Object.keys(forestTypeSymbology).forEach(function (k) {
                buckets[k] = [];
            });
...
function houseIcon(fillHex, strokeHex, strokeWidth, sizePx) {
    ...
    var roofBaseY = pad + 8; // lower roofline = boxier
    var roofHeight = 4; // small trapezoid height
    var bodyTopY = roofBaseY;
    var bodyLeftX = pad + 4;
    var bodyRightX = box - (pad + 4);
    var bodyBottomY = box - (pad + 2);

    // low-pitch trapezoid
    var roof = '<polygon points="'
        + (roofLeftX + 3) + ',' + (roofBaseY - roofHeight) + ' '
        + (roofRightX - 3) + ',' + (roofBaseY - roofHeight) + ' '
...
var woodchipSymbology = {
    "h": {
        label: "Hardwood woodchip supplier",
        color: "#38A700"
    }, // green
    "s": {
        label: "Softwood woodchip supplier",
        color: "#A87001"
    } // brown
};
...
function loadRipley() {
    return fetch("data/ripley.geojson")
        ...
        .then(function (data) {
            ...
            var ripleySw = ripleyIcon("#FFFFFF", "#000000", "#000000", 18).options.html;

            groupedControl.addOverlay(
                ripleyLayer,
                labelWithSvgIcon(ripleySw, "Harvey G. Ripley Heating Plant"),
                "Reference"
            );
            refreshLayerControl();
...
// --------------------
// Load sequence (control order: Ripley, Facilities, Wildlife, Woodshed, Commercial, MiDNR)
// Map order: woodshed always bottom via pane zIndex
// --------------------
var woodshedLayersCache = null;

var woodshedLoad = Promise.all([
    loadGeoJson('data/ws_25_geojson.geojson', style25, 'Woodshed extent - 25 mi distance from Ripley Heating Plant', 'woodshedPane'),
    loadGeoJson('data/ws_50_geojson.geojson', style50, 'Woodshed extent - 50 mi distance from Ripley Heating Plant', 'woodshedPane'),
    loadGeoJson('data/ws_75_geojson.geojson', style75, 'Woodshed extent - 75 mi distance from Ripley Heating Plant', 'woodshedPane'),
    loadGeoJson('data/ws_100_geojson.geojson', style100, 'Woodshed extent - 100 mi distance from Ripley Heating Plant', 'woodshedPane')
]).then(function (layers) {
    woodshedLayersCache = layers;

    // On by default (and always below everything else due to pane zIndex)
    layers[3].addTo(map);
    layers[2].addTo(map);
    layers[1].addTo(map);
    layers[0].addTo(map);

    map.fitBounds(layers[3].getBounds(), {
        padding: [50, 50]
    });
    refreshLayerControl();
});

function addWoodshedToControl() {
    if (!woodshedLayersCache) return;
    groupedControl.addOverlay(woodshedLayersCache[0], labelWithColorSwatch("#B3CDE3", "25 mi distance from Ripley Heating Plant", 0.40), 'Woodshed Extents');
    groupedControl.addOverlay(woodshedLayersCache[1], labelWithColorSwatch("#8C96C6", "50 mi distance from Ripley Heating Plant", 0.40), 'Woodshed Extents');
    groupedControl.addOverlay(woodshedLayersCache[2], labelWithColorSwatch("#8856A7", "75 mi distance from Ripley Heating Plant", 0.40), 'Woodshed Extents');
    groupedControl.addOverlay(woodshedLayersCache[3], labelWithColorSwatch("#810F7C", "100 mi distance from Ripley Heating Plant", 0.40), 'Woodshed Extents');
    refreshLayerControl();
}
...
woodshedLoad
    ...
    .then(function () {
        // Facilities before wildlife in the control
