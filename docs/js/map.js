...
var groupedControl = L.control.groupedLayers(baseMaps, {}, {
    // Desktop: allow collapse/expand; we'll expand it right away so it starts open.
    // Mobile: keep expanded when shown (your CSS controls visibility on mobile).
    collapsed: isMobile ? false : true,
    position: 'topleft'
}).addTo(map);

// Desktop: show it open initially, but keep collapse/expand functionality enabled
if (!isMobile && typeof groupedControl.expand === "function") {
    groupedControl.expand();
}

// Mobile-only Layers toggle button
var layersToggle = L.control({
    position: 'topleft'
});
layersToggle.onAdd = function () {
    ...
    // Prevent map drag/zoom when interacting with the button
...
// IMPORTANT: allow scrolling inside the control on mobile/desktop
var ctlEl = groupedControl.getContainer();
L.DomEvent.disableClickPropagation(ctlEl);
L.DomEvent.disableScrollPropagation(ctlEl);

// Desktop footer toggle: "Open Layer Control" / "Close Layer Control"
(function addDesktopFooterToggle() {
    if (isMobile) return; // mobile uses the separate top-left button

    var container = groupedControl.getContainer();
    if (!container) return;

    // Create footer button inside the control container
    var btn = L.DomUtil.create('button', 'nw-layerctl-footer-toggle', container);
    btn.type = 'button';
    ...
    function syncLabel() {
        btn.textContent = isExpanded() ? 'Close Layer Control' : 'Open Layer Control';
    }
    ...
    // Prevent map interactions when clicking footer
    L.DomEvent.disableClickPropagation(btn);
    L.DomEvent.disableScrollPropagation(btn);
    ...
    // Keep label correct if the control toggles by other means
    map.on('click', syncLabel);
