...
// Keep Woodshed Extents ALWAYS at the bottom, even after toggling.
// --------------------
...
function bringGeoJsonGroupToBack(gj) {
    ...
    } else if (typeof gj.bringToBack === 'function') {
        gj.bringToBack();
    }
}
...
function reorderWoodshedZ() {
    ...
    // Bring-to-back in inner→outer order so final stack is:
    // 100 (bottom) ... 25 (top)
    [0, 1, 2, 3].forEach(function (idx) {
        bringGeoJsonGroupToBack(woodshedLayersCache[idx]);
    });
}
...
loadRipley()
    ...
    .then(function (layers) {
        ...
        // Whenever a woodshed extent is (re)added (e.g., toggled back on), push woodshed to the bottom again.
        layers.forEach(function (lyr) {
            lyr.on('add', function () {
                // Defer so Leaflet finishes adding the internal feature layers first
                setTimeout(reorderWoodshedZ, 0);
            });
        });
        ...
        // Force woodshed beneath everything else (initial load)
        reorderWoodshedZ();
