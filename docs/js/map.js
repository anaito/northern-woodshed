...
// Keep Woodshed Extents ALWAYS at the bottom, and keep their INTERNAL order stable:
...
function bringGeoJsonGroupToBack(gj) {
    ...
    // Prefer FeatureGroup bringToBack if available (cleanest)
    if (typeof gj.bringToBack === 'function') {
        gj.bringToBack();
        return;
    }

    // Fallback: push each child layer back
...
// Reorder ONLY the woodshed extents (among themselves and relative to other vectors)
function reorderWoodshedZ() {
    if (!woodshedLayersCache) return;

    // woodshedLayersCache order is: [25, 50, 75, 100]
    // Calling bringToBack in this order yields final stack:
    // bottom -> top = 100, 75, 50, 25
...
// Robust scheduling so this runs AFTER Leaflet finishes add/remove + canvas redraw bookkeeping
...
loadRipley()
    ...
    .then(function (layers) {
        ...
        // Whenever a woodshed extent is toggled on/off, re-assert BOTH:
        // - woodshed beneath other polygon layers
        // - and internal order: 100 bottom -> 25 top
        ...
        // Force correct woodshed order on first load
